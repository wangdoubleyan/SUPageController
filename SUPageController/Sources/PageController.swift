...
public let SUPageControllerDidMovedToSuperViewNotification = "SUPageControllerDidMovedToSuperViewNotification"
public let SUPageControllerDidFullyDisplayedNotification = "SUPageControllerDidFullyDisplayedNotification"
...
open class PageController: UIViewController, UIScrollViewDelegate, MenuViewDelegate, MenuViewDataSource, PageControllerDelegate, PageControllerDataSource {
    ...
    override open func didReceiveMemoryWarning() {
        ...
        if memoryWarningCount < 3 {
            perform(#selector(PageController.growCachePolicyAfterMemoryWarning), with: nil, afterDelay: 3.0, inModes: [.common])
        }
    }
    ...
    fileprivate func postMovedToSuperViewNotificationWithIndex(_ index: Int) {
        ...
        NotificationCenter.default.post(name: Notification.Name(rawValue: SUPageControllerDidMovedToSuperViewNotification), object: info)
    }
    
    fileprivate func postFullyDisplayedNotificationWithIndex(_ index: Int) {
        guard postNotification else { return }
        let info = ["index": index, "title": titleAtIndex(index)] as [String : Any]
        NotificationCenter.default.post(name: Notification.Name(rawValue: SUPageControllerDidFullyDisplayedNotification), object: info)
    }
    ...
    @objc fileprivate func growCachePolicyAfterMemoryWarning() {
        ...
        perform(#selector(PageController.growCachePolicyToHigh), with: nil, afterDelay: 2.0, inModes: [.common])
    }
